...
// Newsletter subscription helper function
async function subscribeToNewsletter(email, username) {
    try {
        const response = await fetch('https://YOUR_DOMAIN.com/api/newsletter', {
            method: 'POST',
            headers: {
...
if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Subscribe to newsletter (non-blocking)
            subscribeToNewsletter(email, username).catch(err => {
                console.error('Newsletter subscription failed (non-critical):', err);
            });
